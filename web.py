...
def _guess_mdns_hostname() -> str:
    try:
        raw = socket.gethostname().strip().lower()
    except OSError:
        return ""
    slug = re.sub(r"[^a-z0-9]+", "-", raw).strip("-")
    if not slug or slug in ("localhost", "127-0-0-1"):
        return ""
    return f"{slug}.local"


def _build_client_web_registration(port: int) -> dict | None:
    ips = _get_wifi_ipv4_addresses()
    lan = ips[0] if ips else ""
    mdns = _guess_mdns_hostname()
    # Phones on Android often cannot resolve *.local — QR must use LAN IP.
    if lan:
        qr_base = f"http://{lan}:{port}"
    elif mdns:
        qr_base = f"http://{mdns}:{port}"
    else:
        return None
    return {
        "qr_base": qr_base,
...
def _save_client_web_registration_mysql(payload: dict) -> bool:
    params = _app_db_params()
    if not params:
        return False
    qr = str(payload.get("qr_base") or "").strip().rstrip("/")
    if not qr:
        return False
    payload = dict(payload)
    payload["qr_base"] = qr
    payload["registered_at"] = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
...
def _post_client_web_registration_http(payload: dict) -> bool:
    base = _app_server_base()
    if not base:
        return False
    url = f"{base.rstrip('/')}/api/client_web_register.php"
    body = json.dumps(payload).encode("utf-8")
    req = Request(
        url,
        data=body,
...
def _register_client_web_presence(port: int) -> None:
    payload = _build_client_web_registration(port)
    if not payload:
        print("  QR register:         skipped (no LAN IP / hostname)")
        return
    ok_db = _save_client_web_registration_mysql(payload)
    ok_http = _post_client_web_registration_http(payload)
    status = []
    if ok_db:
        status.append("MySQL")
    if ok_http:
        status.append("app API")
    if status:
        print(f"  QR register:         {payload['qr_base']}  ({', '.join(status)})")
    else:
        print(f"  QR register:         failed — check app_server_base / MySQL in settings.json")
        print(f"                       tried {payload['qr_base']}")


def _start_client_web_heartbeat(port: int) -> None:
    import threading
    import time
    ...
    def _loop() -> None:
        while True:
            time.sleep(60)
            try:
                _register_client_web_presence(port)
            except Exception:
                pass

    threading.Thread(target=_loop, daemon=True, name="client-web-qr-heartbeat").start()
...
if __name__ == "__main__":
    ...
    _register_client_web_presence(port)
    _start_client_web_heartbeat(port)
