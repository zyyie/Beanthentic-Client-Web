...
def _storage_urlopen(req: Request, timeout: int = 30):
    """Open Supabase Storage URLs (handles strict SSL on some Windows/Python builds)."""
    import ssl
...
def _list_storage_prefix(prefix: str) -> list[str]:
    ...
    try:
        ...
        with _storage_urlopen(req, timeout=20) as resp:
...
def _upload_bytes_to_supabase(file_bytes: bytes, object_name: str, content_type: str) -> str | None:
    ...
    try:
        ...
        with _storage_urlopen(req, timeout=30) as resp:
            if resp.status >= 400:
                return None
    except HTTPError:
        return None
...
def _fetch_photo_bytes_from_app_server(
    ...
    for base in server_bases:
        if raw and not raw.startswith(("http://", "https://", "data:image/")):
            ...
            if got:
                return got
        got = _http_image_bytes(f"{base}/api/admin_farmer_profile_photo.php?farmer_id={fid}")
        if got:
            return got
        for url in (
...
def _save_local_copy(
    farmer_id: int, data: bytes, ext: str, profile_photo: str = ""
) -> None:
    """Client Web does not keep local farmer photo copies."""
    return None


def _storage_object_name_for_farmer(farmer_id: int, profile_photo: str, ext: str) -> str:
    from config.farmer_profile_photo import _profile_photo_basename, _profile_photo_is_generic

    fid = int(farmer_id or 0)
    basename = _profile_photo_basename(profile_photo)
    if basename and not _profile_photo_is_generic(profile_photo, fid):
        return f"farmers/{basename}"
    return f"farmers/farmer_{fid}{ext}"
...
def backfill_farmer_photos_to_supabase() -> dict:
    ...
    try:
        ...
        for row in rows:
            ...
            got = None
            if _app_server_reachable() and server_bases:
            ...
            if "png" in ctype:
                ...
                ext = ".gif"
            object_name = _storage_object_name_for_farmer(fid, profile_photo, ext)

            from config.farmer_profile_photo import _download_supabase_object

            existing = _download_supabase_object(object_name, 0)

            public_url = _public_url_for_object(object_name)
            if existing and existing[0] == data and profile_photo == public_url:
                already_ok += 1
                continue

            if not existing or existing[0] != data:
                uploaded_url = _upload_bytes_to_supabase(data, object_name, ctype)
                if not uploaded_url:
            ...
            _save_local_copy(fid, data, ext, profile_photo)
...
def purge_all_local_photos() -> int:
    """Remove all cached farmer photos from Client Web disk (photos live in Supabase/app server)."""
    removed = 0
    if not _FARMER_UPLOADS_DIR.is_dir():
        return removed
    for src in _FARMER_UPLOADS_DIR.iterdir():
